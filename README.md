...
| `VatIdSource` | A VAT identification number with a valid check digit, e.g. `DE249051813` or `ATU13585627`, for AT, BE, DE, DK, EE, FI, GB, HR, HU, IE, IT, LT, LU, NL, PL, PT, SE, SI, SK and XI (Northern Ireland). Only countries with a published, calibrated check digit rule are drawn, `new VatIdSource("DE")` pins the country. EL, ES, FR, CY, CZ, LV, MT, BG and RO have not agreed to a publication of their construction rules, so they stay out on purpose: a number that only looks right is worse test data than a missing country. |
...
* `RandomStringSource` and `UrlSource` never used the last character of the allowed character set and the last top level domain of their list. `ColorSource`, `RandomTextSource` and `RandomUtfTextSource` treat their maximum as inclusive now, so `new ColorSource(0, 255)` reaches 255 and a text source asked for 3 to 6 paragraphs can produce 6.
* `ColorSource` applies the alpha given to `SetColorRange(alpha, ...)`, it used to be silently dropped and every color came back fully opaque.
* `CreditCardSource` with `CreditCardType.Random` draws every card type. `Discover` was the last value of the enum and could never come out, the picker stopped at `AmericanExpress`.
* `TimeSpanSource` and `NullableTimeSpanSource` pick uniformly from the inclusive tick range now. The old implementation returned `TimeSpan.MinValue` on every draw of the default range, produced values outside the range that was asked for (1 to 2 hours gave 22 to 58 minutes) and looped forever when the maximum lay before the minimum.
* `DoubleSource`, `FloatSource` and `DecimalSource` interpolate between their bounds without overflowing. `new DoubleSource()` returned infinity on every draw, and `DecimalSource` lost about half of its significant digits on the way through `double`.
* `UrlSource` follows the session seed with its host part as well. The nested string source kept the default seed, so every session produced the same host names no matter what `UseSeed` said. `ExtendedEmailAddressSource` gives its first and last name source separate streams, they used to share one and repeated the same name pairs.
...
* Bug fix: `TimeSpanSource` did not work at all. `new TimeSpanSource()` returned `TimeSpan.MinValue` on every single draw, `new TimeSpanSource(TimeSpan.FromHours(1), TimeSpan.FromHours(2))` returned values between 22 and 58 minutes, so every value was outside the range that was asked for, and a maximum before the minimum sent it into an endless loop. It picks uniformly from the inclusive tick range now, the same way `DateTimeSource` does, and throws an `ArgumentOutOfRangeException` on a reversed range.
* Bug fix: `new DoubleSource()` returned infinity on every draw, because the default range is the whole range of `double` and the old code computed `max - min`. `DoubleSource`, `FloatSource` and `DecimalSource` interpolate as `min * (1 - sample) + max * sample` now, which cannot overflow. `DecimalSource` does it in decimal arithmetic instead of going through `double`, so it keeps its full precision; it used to give numbers like `27879346472004300000000000000` where it now gives `27879346472004256951386422019`. All three throw an `ArgumentOutOfRangeException` when the maximum lies below the minimum.
* Bug fix: `CreditCardSource` never produced a `Discover` card. `CreditCardType.Random` picked with `Random.Next(1, 4)`, which stops at `AmericanExpress`, so the last value of the enum was unreachable; 5000 generated cards contained not a single one. The picker reads the card types from the enum now, so a type added later is drawn as well.
* Bug fix: a handful of sources could not reach the last element of what they draw from. `RandomStringSource` never used the last character of its allowed character set, `UrlSource` never used the last top level domain of its list, and `ColorSource`, `RandomTextSource` and `RandomUtfTextSource` treated their maximum as exclusive, so a colour channel range of 0 to 255 never gave 255 and a text source asked for 3 to 6 paragraphs never gave 6. On top of that `ColorSource` dropped the alpha of `SetColorRange(alpha, ...)` and always returned an opaque colour.
* Bug fix: sources that drive another source internally seed it now. `UrlSource` builds its host with a nested `RandomStringSource` that kept the default seed, so `UseSeed` and `CreateSession(limit, seed)` changed the top level domain but every session saw the same host names. `ExtendedEmailAddressSource` passed one seed to both its first and last name source, so the two followed the same sequence of draws and the same first name came back with the same last name. Both give their nested sources a derived stream of their own now, inside a session and standalone.
